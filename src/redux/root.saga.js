...
import { technologiesListSlice,modulesListSlice,topicsListSlice,subtopicListSlice } from './root.slice'; 
...
export function* technologieslistRequestSaga(action) {
    try {
        const res = yield call(axios.get, endpointTechnoloies,
            action.payload
        );
...
export function* moduleslistRequestSaga(action) {
    try {
        const res = yield call(axios.get, endpointModules,
            action.payload
        );
...
export function* topiclistRequestSaga(action) {
    try {
        const res = yield call(axios.get, endpointTopics,
            action.payload
        );
...
export function* subTopiclistRequestSaga(action) {
    try {
        const res = yield call(axios.get, endpointSubTopics,
            action.payload
        );
 ...
 function* adminWatcher() {
    yield takeEvery(technologiesListSlice.actions.request.type,technologieslistRequestSaga);
    yield takeEvery(modulesListSlice.actions.request.type,moduleslistRequestSaga);
    yield takeEvery(topicsListSlice.actions.request.type,topiclistRequestSaga);
    yield takeEvery(subtopicListSlice.actions.request.type,subTopiclistRequestSaga);
}
